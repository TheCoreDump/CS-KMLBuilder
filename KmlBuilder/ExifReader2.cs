...
namespace ExifLib
{
    ...
    public sealed class ExifReader2 : IDisposable
    {
        ...
        static ExifReader2()
        {
            ...
            {
#if !NETFX_CORE
                var ifdAttribute = (IFDAttribute2)tag.GetCustomAttributes(typeof(IFDAttribute2), false)[0];
#else
        ...
        private ExifReader2(Stream stream, bool leaveOpen, bool internalStream)
        {
            ...
            try
            {
                ...
                if (!stream.CanSeek)
                    throw new ExifLibException2("ExifLib requires a seekable stream");
                ...
                if (ReadUShort() != 0xFFD8)
                    throw new ExifLibException2("File is not a valid JPEG");
                ...
                {
                    throw new ExifLibException2("Unable to locate EXIF content", ex);
                }
                ...
                {
                    throw new ExifLibException2("Error indexing EXIF tags", ex);
                }
            }
            catch
            {
        ...
        private byte GetTIFFFieldLength(ushort tiffDataType)
        {
            switch (tiffDataType)
            {
                ...
                case 12:
                    return 8;
                default:
                    throw new ExifLibException2(string.Format("Unknown TIFF datatype: {0}", tiffDataType));
            }
        }
        ...
        /// <returns></returns>
        public byte[] ReadBytes(ushort tiffOffset, int byteCount)
        {
        ...
        private void ReadToExifStart()
        {
            ...
            while (((markerStart = _reader.ReadByte()) == 0xFF) && (markerNumber = _reader.ReadByte()) != 0xE1)
            {
                ...
                if (_stream.Position != expectedPosition)
                    throw new ExifLibException2(string.Format("Supplied stream of type {0} reports CanSeek=true, but fails to seek", _stream.GetType()));
            }

            // It's only success if we found the 0xFFE1 marker
            if (markerStart != 0xFF || markerNumber != 0xE1)
                throw new ExifLibException2("Could not find Exif data block");
        }
        ...
        private void CreateTagIndex()
        {
            ...
            if (ReadString(4) != "Exif")
                throw new ExifLibException2("Exif data not found");

            // 2 zero bytes
            if (ReadUShort() != 0)
                throw new ExifLibException2("Malformed Exif data");
            ...
            if (ReadUShort() != 0x002A)
                throw new ExifLibException2("Error in TIFF data");
        ...
        public long GetTagOffset(ExifTags tag)
        {
            if (_ifd0PrimaryCatalogue.ContainsKey((ushort)tag))
                return _ifd0PrimaryCatalogue[(ushort)tag];

            if (_ifdExifCatalogue.ContainsKey((ushort)tag))
                return _ifdExifCatalogue[(ushort)tag];

            if (_ifdGPSCatalogue.ContainsKey((ushort)tag))
                return _ifdGPSCatalogue[(ushort)tag];
        ...
        private byte[] GetTagBytes(Dictionary<ushort, long> tagDictionary, ushort tagID, out ushort tiffDataType, out uint numberOfComponents)
        {
            ...
            if (currentTagID != tagID)
                throw new ExifLibException2("Tag number not at expected offset");
    ...
    public class ExifLibException2 : Exception
    {
        public ExifLibException2()
        {
        }

        public ExifLibException2(string message)
            : base(message)
        {
        }

        public ExifLibException2(string message, Exception innerException)
    ...
    internal class IFDAttribute2 : Attribute
    {
        public readonly IFD IFD;

        public IFDAttribute2(IFD ifd)
        {
            IFD = ifd;
        }
    }
